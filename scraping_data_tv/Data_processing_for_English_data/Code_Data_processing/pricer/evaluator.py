...
def plot_training_history(history, title="Deep Neural Network"):
    """Plot train/val loss, val MAE, and learning rate from training history."""
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(size=8),
            hovertemplate="Epoch %{x}<br>MAE: $%{y:,.2f}<extra></extra>",
        ),
        row=2,
        col=1,
    )
    ...
    fig.update_layout(
        title=f"{title} Training History | Final Train: {final_train:.4f} Val: {final_val:.4f} MAE: ${final_mae:,.2f}",
        height=900,
        width=1000,
        template="plotly_white",
        showlegend=True,
