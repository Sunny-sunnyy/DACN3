...
class ScrapedDeal:
    ...
    def __init__(self, entry: Dict[str, str]):
        ...
        # Try to scrape additional details from the page
        try:
            ...
            if content_div:
                ...
                # Fallback: use summary from RSS if content-section not found
                self.details = self.summary
                self.features = ""
        except Exception as e:
            # Fallback on any error (timeout, connection error, etc.)
            self.details = self.summary
            self.features = ""
