...
def html_for(log_data):
    output = '<br>'.join(log_data)
    return f"""
    <div id="logScroll" style="height: 400px; overflow-y: auto; border: 1px solid #ccc; background-color: #222229; padding: 10px; font-size: 16px;">
    {output}
    </div>
    <script>
    (function() {{
        ...
        with gr.Blocks(title="The Price is Right", fill_width=True) as ui:
            ...
            def table_for(opps):
                return [[
                    ...
                    f"${opp.estimate:.2f}",
                    f"${opp.discount:.2f}",
                    f'<a href="{opp.deal.url}" target="_blank">View Deal</a>',
                ] for opp in opps]
